...
// export default function Landing() {
//   const [wishlist, setWishlist] = useState([]);
//   const [wines, setWines] = useState([]); // Initialize with empty array for dynamic data
//   const router = useRouter();
//   const [showAllWines, setShowAllWines] = useState(false);
//   const [searchQuery, setSearchQuery] = useState("");
...
//   const types = [
//     ...new Set(
//       wines.map((wine) => wine.type).filter((type) => type && type.trim() !== "")
//     ),
//   ];
//   const regions = [
//     ...new Set(
//       wines.map((wine) => wine.region).filter((region) => region && region.trim() !== "")
//     ),
//   ];
//   const volumes = [
//     ...new Set(
//       wines.map((wine) => wine.volume).filter((volume) => volume && volume.trim() !== "")
//     ),
//   ];
//   const alcoholContents = [
//     ...new Set(
//       wines
//         .map((wine) => wine.alcohol_content.replace('%', ''))
//         .filter((alcohol) => alcohol && alcohol.trim() !== "")
//     ),
//   ];

//   useEffect(() => {
//     // Fetch wine data from an API or external source
//     const fetchWines = async () => {
//       try {
//         const response = await fetch("/api/wines");  // Adjust to your API endpoint
//         const data = await response.json();
//         setWines(data);  // Store the fetched data
//       } catch (error) {
...
//     )
//     .filter((wine) =>
//       appliedFilters.type ? wine.type.toLowerCase() === appliedFilters.type.toLowerCase() : true
//     )
//     .filter((wine) =>
//       appliedFilters.region ? wine.region.toLowerCase() === appliedFilters.region.toLowerCase() : true
//     )
//     .filter((wine) =>
//       appliedFilters.volume ? wine.volume === appliedFilters.volume : true
//     )
//     .filter((wine) =>
//       appliedFilters.alcoholContent ? wine.alcohol_content === appliedFilters.alcoholContent : true
//     );
...
//             <select
//               onChange={(e) => setFilters({ ...filters, alcoholContent: e.target.value })}
...
//             className="mt-4 bg-[#004e89] text-white px-8 py-3 rounded-lg shadow-lg hover:bg-[#003765]"
//           >
//             Apply Filters
//           </button>
//         </div>
...
//                 </div>
//               </div>
//               <p className="font-semibold text-lg">{wine.title}</p>
//               <p className="text-sm text-gray-500">{wine.region}</p>
//               <p className="font-semibold">{wine.price}</p>
//             </div>
//           ))}
//         </div>
...
//           <button
//             onClick={() => setShowAllWines(!showAllWines)}
//             className="bg-[#004e89] text-white px-8 py-3 rounded-lg shadow-lg hover:bg-[#003765]"
//           >
//             {showAllWines ? "Show Less" : "Show More"}
//           </button>
//         </div>
//       </main>
//     </div>
